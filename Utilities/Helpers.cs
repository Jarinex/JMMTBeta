...
namespace JMMT.Utilities
{
    ...
    //
    // - don't import HarmonyLib, ever! It has an extension method called "Add" that can
    ...
    // - don't use HarmonyLib.Traverse! It silently ignores failures. For example, you try to set
    ...
    public static class ExtensionMethods
    {
        ...
        {
            return maxAmountInfo.GetValue(resource);
        }
        private static void setMaxAmount(BlueprintAbilityResource resource, object value)
        {
    ...
    public static class Helpers
    {
        ...
        {
            HarmonyLib.AccessTools.Field(obj.GetType(), name).SetValue(obj, Helpers.CreateString($"{obj.name}.{name}", value));
        }
        ...
        {
            return new FastInvoke(HarmonyLib.MethodInvoker.GetHandler(HarmonyLib.AccessTools.Method(type, name)));
        }
        ...
        {
            return new FastInvoke(HarmonyLib.MethodInvoker.GetHandler(HarmonyLib.AccessTools.Method(type, name, args, typeArgs)));
        }
